      ...
      <div className="mb-6 space-y-3">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <button onClick={() => setFilterTerbaru(!filterTerbaru)}
            className={`text-[11px] sm:text-xs font-semibold px-2.5 sm:px-3 py-1.5 rounded-full border transition ${
          ...
          <div className="flex items-center gap-1">
            {['habis', 'rendah', 'tersedia'].map((s) => (
              <button key={s} onClick={() => setFilterStok(filterStok === s ? '' : s)}
                className={`text-[11px] sm:text-xs font-semibold px-2 sm:px-3 py-1.5 rounded-full border transition ${
                  filterStok === s ? 'bg-green-500 text-white border-green-500' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300'
                }`}>
                {s === 'habis' ? 'Habis' : s === 'rendah' ? 'Rendah' : 'Ada'}
              </button>
            ))}
          </div>
              ...
              <div key={product.id} className="bg-white rounded-lg border border-gray-100 overflow-hidden hover:shadow-md transition-all hover:-translate-y-0.5 group flex flex-col">
                ...
                <div className="p-3 flex flex-col gap-2 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {product.kode_item && (
                      <p className="hidden sm:block text-[11px] font-semibold text-gray-500 shrink-0 mt-0.5">{product.kode_item}</p>
                    )}
                  </div>
                  ...
                  >
                    ...
                    ) : <><span className="hidden sm:inline">Tambah ke Keranjang</span><span className="sm:hidden">+ Keranjang</span></>}
                  </button>
                </div>
              </div>
            )
